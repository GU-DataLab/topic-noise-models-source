...
public class TopicTrainer {
    ...
    static CommandOption.String seedTopicsFile = new CommandOption.String(TopicTrainer.class, "seed-topics", "FILENAME", true, null,
		 "The filename in which seed topics are found.  " +
		 "By default this is null, indicating that no seed topics are given.", null);

    static CommandOption.String seedGpuWeightsFile = new CommandOption.String(TopicTrainer.class, "seed-gpu-weights", "FILENAME", true, null,
		 "The filename in which seed gpu weights are found.  " +
		 "By default this is null, indicating that no seed gpu weights are given.", null);

    static CommandOption.String seedTopicsFileDelimiter = new CommandOption.String(TopicTrainer.class, "seed-topics-delimiter", "STRING", false, "COMMA",
		 "The delimiter for seed topic words.", null);
    ...
    static CommandOption.Integer samplingScheme = new CommandOption.Integer(TopicTrainer.class, "samplingScheme", "INTEGER", true, 1,
         "The sampling scheme used by the topic model.  0 = normal, 1 = oversampling of seeds, 2 = GPU sampling of seeds", null);

    static CommandOption.Integer overSamplingFactor = new CommandOption.Integer(TopicTrainer.class, "overSamplingFactor", "INTEGER", true, 1,
		 "overSamplingFactor parameter: amount to oversample a seed word by in sampling scheme 1",null);
	...
	public static void main (String[] args) throws java.io.IOException {
		...
		if (inputModelFilename.value != null) {
		    ...
		    ArrayList<ArrayList<String>> seeds = new ArrayList<ArrayList<String>>();
		    if (seedTopicsFile != null) {
		        Scanner inSeedTopics = new Scanner(new File(seedTopicsFile.value));
		        // Need to figure this out eventually so that people can give seed topics with different delimiters.
// 		        String delimiterCode = seedTopicsFileDelimiter.value;
//                 String delimiter;
//                 logger.warning(delimiterCode);
// //                 logger.warning(delimiterCode.equals("SPACE"));
//                 if (delimiterCode.equals("SPACE")) {
//                     delimiter = " ";
//                 } else {
//                     delimiter = "," ;
//                 }
//                 logger.warning(delimiter);
		    ...
		    ArrayList<ArrayList<Double>> seedGpuWeights = new ArrayList<ArrayList<Double>>();
		    if (seedGpuWeightsFile != null) {
		        Scanner inSeedTopics = new Scanner(new File(seedGpuWeightsFile.value));
		        while(inSeedTopics.hasNext()) {
		            String line = inSeedTopics.next();

		            ArrayList<String> weightStrings = new ArrayList(Arrays.asList(line.split(",")));
		            ArrayList<Double> weights = new ArrayList<Double>();
		            for (int i = 0; i < weightStrings.size(); i++) {
		                weights.add(Double.parseDouble(weightStrings.get(i)));
		            }
		            if (weights.size() > 0) {
		                seedGpuWeights.add(weights);
		            }
		        }
		        inSeedTopics.close();
		    }
			topicModel = new ParallelTopicModel (numTopics.value, alpha.value, beta.value, samplingScheme.value, overSamplingFactor.value, seeds, seedGpuWeights);
		}
