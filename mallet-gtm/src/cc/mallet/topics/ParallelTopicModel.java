...
public class ParallelTopicModel implements Serializable {
    ...
    public int samplingScheme;
    public int overSamplingFactor;
    public ArrayList<ArrayList<Double>> seedGpuWeights;
	...
	public ArrayList<ArrayList<String>> seeds;
	...
	public ParallelTopicModel (int numberOfTopics) {
		this (numberOfTopics, numberOfTopics, DEFAULT_BETA, 1, 1, new ArrayList<ArrayList<String>>(), new ArrayList<ArrayList<Double>>());
	}

    public ParallelTopicModel (int numberOfTopics, double alphaSum, double beta) {
		this (newLabelAlphabet (numberOfTopics), alphaSum, beta, 1, 1, new ArrayList<ArrayList<String>>(), new ArrayList<ArrayList<Double>>());
	}

	public ParallelTopicModel (int numberOfTopics, double alphaSum, double beta, int samplingScheme) {
		this (newLabelAlphabet (numberOfTopics), alphaSum, beta, samplingScheme, 1, new ArrayList<ArrayList<String>>(), new ArrayList<ArrayList<Double>>());
	}

    public ParallelTopicModel (int numberOfTopics, double alphaSum, double beta, int samplingScheme, int overSamplingFactor) {
		this (newLabelAlphabet (numberOfTopics), alphaSum, beta, samplingScheme, overSamplingFactor, new ArrayList<ArrayList<String>>(), new ArrayList<ArrayList<Double>>());
	}

	public ParallelTopicModel (int numberOfTopics, double alphaSum, double beta, int samplingScheme, ArrayList<ArrayList<String>> seeds, ArrayList<ArrayList<Double>> seedGpuWeights) {
		this (newLabelAlphabet (numberOfTopics), alphaSum, beta, samplingScheme, 1, seeds, seedGpuWeights);
	}

    public ParallelTopicModel (int numberOfTopics, double alphaSum, double beta, int samplingScheme, int overSamplingFactor, ArrayList<ArrayList<String>> seeds, ArrayList<ArrayList<Double>> seedGpuWeights) {
		this (newLabelAlphabet (numberOfTopics), alphaSum, beta, samplingScheme, overSamplingFactor, seeds, seedGpuWeights);
	}
	...
	public ParallelTopicModel (LabelAlphabet topicAlphabet, double alphaSum, double beta, int samplingScheme, int overSamplingFactor, ArrayList<ArrayList<String>> seeds, ArrayList<ArrayList<Double>> seedGpuWeights) {
		...
		this.seedGpuWeights = seedGpuWeights;
// 		logger.info("seedGpuWeights Length: " + seedGpuWeights.size() + " seedGpuWeights: " + seedGpuWeights);
// 		logger.info("seeds Length: " + seeds.size() + " seeds: " + seeds);
		...
		logger.info("Mallet GTM: " + numTopics + " topics, " + topicBits + " topic bits, " +
					Integer.toBinaryString(topicMask) + " topic mask");
//         logger.info("GPU Weights: " + seedGpuWeights);
	}
	...
	public int seedTopicIndex(String word) {
	    for (int i = 0; i < seeds.size(); i++) {
	        if (seeds.get(i).contains(word)) {
	            return i;
	        }
	    }
	    return -1;
	...
	public void buildInitialTypeTopicCounts () {
		...
		tokensPerTopic = new int[numTopics];
//         logger.info("typeTopicCounts: " + typeTopicCounts.length + " types: " + alphabet.size() + "\n" + alphabet + "\n\n" + seeds);
		...
		for (TopicAssignment document : data) {
			...
			for (int position = 0; position < tokens.size(); position++) {
				...
				int seedTopic = seedTopicIndex(word);
				if (seedTopic != -1) {
				    topic = seedTopic;
				}
		...
		if (numThreads > 1) {
			...
			for (int thread = 0; thread < numThreads; thread++) {
				...
				runnables[thread] = new WorkerRunnable(numTopics,
													   ...
													   samplingScheme, overSamplingFactor,
													   seeds, seedGpuWeights);
