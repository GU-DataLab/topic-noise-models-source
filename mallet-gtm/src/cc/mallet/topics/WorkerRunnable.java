...
public class WorkerRunnable implements Runnable {
	...
	protected int samplingScheme;
	protected int overSamplingFactor;
	protected ArrayList<ArrayList<String>> seeds;
	protected ArrayList<ArrayList<Double>> seedGpuWeights;
	...
	public WorkerRunnable (int numTopics,
						   ...
						   ArrayList<ArrayList<Double>> seedGpuWeights) {
...
// 						   Integer.toBinaryString(topicMask) + " topic mask");
            ...
            for (int position = 0; position < tokens.size(); position++) {
				...
				while (currentTypeTopicCounts[index] > 0 && currentTopic != topic) {
					...
					if (index == currentTypeTopicCounts.length) {
						System.err.println("overflow on type " + type);
					}
    ...
    private int seedTopicIndex(String word) {
    ...
    private int seedTopicWordIndex(String word) {
	    for (int i = 0; i < seeds.size(); i++) {
	        if (seeds.get(i).contains(word)) {
	            return seeds.get(i).indexOf(word);
	        }
	    }
	    return -1;
	}
	...
	protected void sampleTopicsForOneDoc (FeatureSequence tokenSequence,
		...
		for (int position = 0; position < docLength; position++) {
            ...
            int seedTopicIdx = seedTopicWordIndex(word);

			if (sample < topicTermMass || seedTopic != -1) {
				//topicTermCount++;
...
// 				System.err.println("newTopic: " + newTopic + " seedTopic: " + seedTopic + " i: " + i + " currentValue: " + currentValue);

				if (seedTopic != -1) {
                    if (samplingScheme == 0) {
                        // normal sampling
                        currentTypeTopicCounts[i] = ((currentValue + 1) << topicBits) + newTopic;
                    } else if (samplingScheme == 1) {
                        // Seed word oversampling
                        currentTypeTopicCounts[i] = ((currentValue + overSamplingFactor) << topicBits) + newTopic;
                    } else if (samplingScheme == 2) {
                        // GPU sampling
                        ArrayList<String> seedTopicWords = seeds.get(i);
                        for (String seedWord : seedTopicWords) {
                            int seedType = alphabet.lookupIndex(seedWord, false);
                            if (seedType != -1) {
                                int[] seedTypeTopicCounts = typeTopicCounts[seedType];
//                                 System.err.println("seedTypeTopicCounts: "+seedTypeTopicCounts.length + " for word: "+ seedWord + " seedType: "+ seedType);
                                int seedCurrentValue = seedTypeTopicCounts[i] >> topicBits;
                                double gpuWeight = seedGpuWeights.get(seedTopic).get(seedTopicWordIndex(seedWord));
                                seedTypeTopicCounts[i] = ((seedCurrentValue + (int) (gpuWeight*overSamplingFactor)) << topicBits) + newTopic;
                            }
//                             System.err.println("seedType: " + seedType + " seedWord: " + seedWord + " \n" + alphabet + " \n" + typeTopicCounts.length + " " + alphabet.size());
                        }
                        // GET SEED GPU WEIGHT AND MULTIPLY OVERSAMPLING FACTOR
                        double gpuWeight = seedGpuWeights.get(seedTopic).get(seedTopicIdx);
                        currentTypeTopicCounts[i] = ((currentValue + (int) (gpuWeight*overSamplingFactor)) << topicBits) + newTopic;
                    }
                } else {
                    currentTypeTopicCounts[i] = ((currentValue + 1) << topicBits) + newTopic;
                }
