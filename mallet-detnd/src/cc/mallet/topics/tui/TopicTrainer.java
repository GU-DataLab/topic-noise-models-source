...
public class TopicTrainer {
    ...
    static CommandOption.String alphaArrayFile = new CommandOption.String(TopicTrainer.class, "output-alpha-array", "FILENAME", true, null,
         "The filename in which to write the alpha array at the end of the iterations.  " +
         "By default this is null, indicating that no file will be written.", null);

    static CommandOption.String twDistInfile = new CommandOption.String(TopicTrainer.class, "tw-dist-infile", "FILENAME", true, null,
         "The filename containing the initial topic-word distribution.  " +
         "By default this is null, indicating that no file will be loaded.", null);

    static CommandOption.String noiseDistInfile = new CommandOption.String(TopicTrainer.class, "noise-dist-infile", "FILENAME", true, null,
         "The filename containing the initial noise distribution.  " +
         "By default this is null, indicating that no file will be written.", null);

    static CommandOption.String alphaArrayInFile = new CommandOption.String(TopicTrainer.class, "alpha-array-infile", "FILENAME", true, null,
         "The filename containing the initial alpha distribution.  " +
         "By default this is null, indicating that no file will be written.", null);

    static CommandOption.String betaFile = new CommandOption.String(TopicTrainer.class, "output-beta", "FILENAME", true, null,
         "The filename in which to write the beta parameter at the end of the iterations.  " +
         "By default this is null, indicating that no file will be written.", null);
    ...
    public static void main (String[] args) throws java.io.IOException {
        ...
        if (inputModelFilename.value != null) {
		    ...
		    HashMap<String, Integer> initialNoiseDist = new HashMap<String, Integer>();
		    if (noiseDistInfile != null && noiseDistInfile.value != null) {
		        Scanner inDistWords = new Scanner(new File(noiseDistInfile.value));
		        while(inDistWords.hasNext()) {
		            String line = inDistWords.next();
		            List<String> w = new ArrayList(Arrays.asList(line.split(", ")));
		            if (w.size() > 1) {
                        String key = w.get(0);
                        int val = Integer.parseInt(w.get(1));
		                initialNoiseDist.put(key, val);
		            }
		        }
		    }
            HashMap<String, Integer[]> initialTopicWordDist = new HashMap<String, Integer[]>();
		    if (twDistInfile != null && twDistInfile.value != null) {
		        Scanner inDistWords = new Scanner(new File(twDistInfile.value));
		        while(inDistWords.hasNext()) {
		            String line = inDistWords.next();
		            List<String> w = new ArrayList(Arrays.asList(line.split("\t")));
		            if (w.size() > 2) {
                        int topic = Integer.parseInt(w.get(0));
                        String word = w.get(1);
                        int val = Integer.parseInt(w.get(2));
                        if (!initialTopicWordDist.containsKey(word)) {
                             initialTopicWordDist.put(word, new Integer[30]);
                        }
		                Integer[] al = initialTopicWordDist.get(word);
                        al[topic] = (al[topic] + val);
                        initialTopicWordDist.replace(word, al);
		            }
		        }
		    }
            if (alphaArrayInFile != null && alphaArrayInFile.value != null) {
                double[] alphaArray = new double[numTopics.value];
                Scanner inArray = new Scanner(new File(alphaArrayInFile.value));
                int idx = 0;
		        while(inArray.hasNext()) {
                    String line = inArray.next();
                    double x = Double.parseDouble(line.trim());
                    alphaArray[idx] = x;
                    idx++;
                }
                topicModel = new ParallelTopicModel (numTopics.value, alphaArray, beta.value, skew.value, tau.value,
                        closeWords, initialNoiseDist, initialTopicWordDist);
            } else {

                topicModel = new ParallelTopicModel (numTopics.value, alpha.value, beta.value, skew.value, tau.value,
                        closeWords, initialNoiseDist, initialTopicWordDist);
            }
        }
...
//         }
        ...
        if (alphaArrayFile.value != null && outputStateInterval.value == 0) {
            topicModel.printAlphaArray (new File(alphaArrayFile.value));
        }

        if (betaFile.value != null && outputStateInterval.value == 0) {
            topicModel.printBeta (new File(betaFile.value));
        }
