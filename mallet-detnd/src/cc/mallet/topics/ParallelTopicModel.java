...
public class ParallelTopicModel implements Serializable {
    ...
    public int[][] typeTopicDist; // indexed by <feature index, topic index> THIS IS USED TO LOAD IN THE INITIAL DISTRIBUTION, IF ANY
    ...
    public HashMap<String, Integer> initialNoiseDist;
    public HashMap<String, Integer[]> initialTopicWordDist;
    ...
    public ParallelTopicModel (int numberOfTopics, double alphaSum, double beta, double skew,
                               int tau, HashMap<String, List<String>> closeWords,
                               HashMap<String, Integer> initialNoiseDist,
                               HashMap<String, Integer[]> initialTopicWordDist) {
        this (newLabelAlphabet (numberOfTopics), alphaSum, beta, skew, tau, closeWords, initialNoiseDist, initialTopicWordDist);
    }

	public ParallelTopicModel (int numberOfTopics, double[] alpha, double beta, double skew,
	                           int tau, HashMap<String, List<String>> closeWords,
                               HashMap<String, Integer> initialNoiseDist,
                               HashMap<String, Integer[]> initialTopicWordDist) {
        this (newLabelAlphabet (numberOfTopics), alpha, beta, skew, tau, closeWords, initialNoiseDist, initialTopicWordDist);
    }
    ...
    public ParallelTopicModel (LabelAlphabet topicAlphabet, double alphaSum, double beta,
        ...
        this.beta = beta;
        this.initialNoiseDist = initialNoiseDist;
        this.initialTopicWordDist = initialTopicWordDist;
        this.tau = tau;
        ...
        logger.info("Mallet deTND: " + numTopics + " topics, " + topicBits + " topic bits, " +
    ...
    public ParallelTopicModel (LabelAlphabet topicAlphabet, double[] alpha, double beta, double skew,
                               int tau, HashMap<String, List<String>> closeWords,
                               HashMap<String, Integer> initialNoiseDist,
                               HashMap<String, Integer[]> initialTopicWordDist) {
    ...
    public void buildInitialNoiseDistribution() {
        noiseDistribution = new int[numTypes];
        if (initialNoiseDist.size() == 0) {
            return;
        }
        Iterator it = initialNoiseDist.entrySet().iterator();
        while (it.hasNext()) {
            HashMap.Entry<String, Integer> pair = (HashMap.Entry)it.next();
            String word = pair.getKey();
            int count = pair.getValue().intValue();
            int type = alphabet.lookupIndex(word, false);
            noiseDistribution[type] += count;
        }
    }

    public void buildTypeTopicDist() {
        /*
            This builds the initial distribution on types instead of string words for use in building typetopiccounts,
            which is used in the actual model
        */
        typeTopicDist = new int[numTypes][numTopics];
        if (initialTopicWordDist.size() == 0) {
            return;
        }
        Iterator it = initialTopicWordDist.entrySet().iterator();
        while (it.hasNext()) {
            HashMap.Entry<String, Integer[]> pair = (HashMap.Entry) it.next();
            String word = pair.getKey();
            Integer[] topicCounts = pair.getValue();
            if (topicCounts.length == numTopics) {
                int type = alphabet.lookupIndex(word, false);
                if (type > -1) {
                    typeTopicDist[type] = new int[topicCounts.length];
                    for (int i = 0; i < topicCounts.length; i++) {
                        typeTopicCounts[type][i] = topicCounts[i].intValue();
                    }
                }
            }
        }
    ...
    public void buildInitialTypeTopicCounts () {
        ...
        for (int type = 0; type < numTypes; type++) {
            int initialTotal = 0;
            if (typeTopicDist[type] != null) {
                initialTotal = Arrays.stream(typeTopicDist[type] ).sum();
            }
            if (typeTotals[type] + initialTotal > maxTypeCount) { maxTypeCount = typeTotals[type] + initialTotal; }
            typeTopicCounts[type] = new int[ Math.min(numTopics, typeTotals[type] + initialTotal) ];
        }
        ...
        for (TopicAssignment document : data) {
            ...
            for (int position = 0; position < tokens.size(); position++) {
                ...
                int initialValue = typeTopicDist[type][topic];
                int thisValue = 1 + initialValue;
                if (currentValue > maxTopicValue) { maxTopicValue = currentValue; }
                if (!noiseRoll(type, currentValue + thisValue)) {
                    typeTopicDist[type][topic] = 0;
                    if (currentValue == 0) {
                        // new value is 1, so we don't have to worry about sorting
                        //  (except by topic suffix, which doesn't matter)

                        currentTypeTopicCounts[index] =
                                (thisValue << topicBits) + topic;
                    } else {
                        currentTypeTopicCounts[index] =
                                ((currentValue + thisValue) << topicBits) + topic;
...
//                 assert (noiseTokens1 == noiseTokens2) : "noise tokens do not match " + noiseTokens1 + ", " + noiseTokens2;
    ...
    public void printAlphaArray(File file) throws IOException {
    ...
    public void printBeta(File file) throws IOException {
        PrintStream out = new PrintStream (file);
        out.append(beta + "\n");
        out.close();
    }
